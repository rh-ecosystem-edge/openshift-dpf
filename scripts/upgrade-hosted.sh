...
extract_version() {
	echo "$1" | grep -oP ':\K[0-9]+\.[0-9]+\.[0-9]+' || true
}
...
handle_machine_os_url() {
	...
	if [[ -z "${current_url}" && -z "${HOSTED_UPGRADE_MACHINE_OS_URL:-}" ]]; then
		return 0
	fi
...
ensure_ignition_cleaned_up() {
	...
	if [[ "${i}" -gt "${UPGRADE_START_POLL_RETRIES}" ]]; then
		echo "ERROR: DPFHCPProvisioner did not leave Ready phase after ${UPGRADE_START_POLL_RETRIES} attempts — upgrade may not have started." >&2
		return 1
	fi

	for ((j = 1; j <= IGNITION_CLEANUP_POLL_RETRIES; j++)); do
		local cm_output
		cm_output=$(oc get configmap -n "${DPU_CLUSTER_NAMESPACE}" "${IGNITION_CM}" --ignore-not-found -o name)
		if [[ -z "${cm_output}" ]]; then
...
ensure_ignition_recreated() {
	...
	actual_image=$(oc get configmap -n "${DPU_CLUSTER_NAMESPACE}" "${IGNITION_CM}" \
		-o go-template="{{index .metadata.annotations \"${IGNITION_RELEASE_ANNOTATION_NEW}\"}}" 2>/dev/null || true)
	if [[ -z "${actual_image}" ]]; then
		actual_image=$(oc get configmap -n "${DPU_CLUSTER_NAMESPACE}" "${IGNITION_CM}" \
			-o go-template="{{index .metadata.annotations \"${IGNITION_RELEASE_ANNOTATION_OLD}\"}}" 2>/dev/null || true)
	fi
...
resolve_target_version() {
	...
	if [[ -z "${HOSTED_UPGRADE_TARGET_VERSION:-}" ]]; then
		local current_z="${current_version##*.}"
...
check_already_upgraded() {
	if [[ "${CURRENT_VERSION}" != "${HOSTED_UPGRADE_TARGET_VERSION}" ]]; then
		return 1
	fi
	local phase
	phase=$(get_provisioner_field '{.status.phase}')
	if [[ "${phase}" == "Ready" ]]; then
		echo "Already at target version ${HOSTED_UPGRADE_TARGET_VERSION}."
		ensure_ignition_recreated
		return 0
	fi
	echo "Already patched to ${HOSTED_UPGRADE_TARGET_VERSION} but phase=${phase}, resuming monitoring..."
	follow_upgrade_to_completion && ensure_ignition_recreated
}

resolve_target_version
resolve_ignition_refs

if check_already_upgraded; then
	exit 0
fi
