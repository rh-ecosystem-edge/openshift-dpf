...
# Find number of DPU host worker nodes on management cluster which are ready
dpu_host_worker_list=$(oc get nodes --no-headers -l node-role.kubernetes.io/worker  --kubeconfig="${mgmt_kubecfg}" | grep -v "control-plane" | awk '$2=="Ready" && $3=="worker" {print $1}' | xargs)

echo -e "\nChecking how many DPU host worker nodes are in Ready Phase on management cluster"
if [ -z "${dpu_host_worker_list}" ]; then
  echo "No DPU host worker nodes found in Ready Phase on management cluster"
  exit 1
fi
