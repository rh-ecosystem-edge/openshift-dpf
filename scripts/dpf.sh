...
function deploy_dpf_hcp_bridge_operator() {
    ...
    if helm upgrade --install dpf-hcp-bridge-operator \
        ...
        --values "${HELM_CHARTS_DIR}/dpf-hcp-bridge-operator-values.yaml" \
...
function create_dpfhcpbridge_cr() {
    ...
    # Add virtualIP if HYPERSHIFT_API_IP is set and control plane is HighlyAvailable
    if [ -n "${HYPERSHIFT_API_IP}" ] && [ "${control_plane_policy}" == "HighlyAvailable" ]; then
        cat >> "${cr_file}" << EOF
...
>>>>>>> parent of 0f8fdad (Revert "feat: integrate dpf-hcp-bridge-operator to replace hypershift CLI and kubeconfig injection workflow")
