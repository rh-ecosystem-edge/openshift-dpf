...
help:
	...
	@echo "  deploy-metallb   - Deploy MetalLB operator for LoadBalancer support (only if HYPERSHIFT_API_IP is set; IPAddressPool/L2Advertisement managed by dpf-hcp-bridge-operator)"
	...
	@echo "                          If set: Deploys MetalLB operator and uses LoadBalancer for Hypershift API (dpf-hcp-bridge-operator manages IPAddressPool/L2Advertisement)"
